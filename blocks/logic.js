...
Blockly.Blocks['logic_compare'] = {
     ...
     */
    init : function() {
        var OPERATORS = this.RTL ? [ [ '=', 'EQ' ], [ '\u2260', 'NEQ' ], [ '>', 'LT' ], [ '\u2265', 'LTE' ], [ '<', 'GT' ], [ '\u2264', 'GTE' ] ] : [
    ...
    /**
     * Modify this block to have a appropriate operator. Only used in Roberta
     * blocks.
     * 
     * @param {String}
     *            operator
    ...
    /**
     * Called whenever anything on the workspace changes. Prevent mismatched
     * types from being compared.
     * 
...
Blockly.Blocks['robControls_if'] = {
    ...
    updateShape_ : function(num) {
        ...
        if (num == 1) {
            ...
            lc.render();
            lc.updateShape('BOOL');
            var connection = this.getInput('IF' + this.elseIfCount_).connection;
            connection.connect(lc.outputConnection);
            var s = this.workspace.newBlock('math_number');
            s.initSvg();
            s.render();
